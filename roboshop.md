...
# 1. Install NodeJS, By default NodeJS 16 is available, We would like to enable 20 version and install list.

**You can list modules by using `dnf module list nodejs`**

# 1.a. Disable current module & Enable required module
```shell 
...
# 2.b Lets setup an app directory. 

```shell
mkdir /app 
```

# 2.c Download the application code to created app directory. 
```shell
...
# 2.d Lets download the dependencies. 
...
# 2.e Setup SystemD Catalogue Service 
create file by using **`vim /etc/systemd/system/catalogue.service`**
...
# 2.g Enable and Start the service.
...
# 3. For the application to work fully functional we need to load schema to the Database. Additonally we are going to have master data to be seeded for the applications to work. Here in this case it is list of products we want to sale.
Schemas are usually part of application code and developer will provide them as part of development.
Master data are usually provided by business operations team.
# 3.a To load schema / master data we need to install mongodb client and then we can load it.
# 3.b To have mongo client installed we have to setup MongoDB repo and install mongodb-client. You can create file using
...
# 3.c Load Master Data of the List of products we want to sell and their quantity information also there in the same master data. 
...
# 3.d Use below commands to check data is loaded into mongodb or not
Connect to MongoDB
```
mongosh --host MONGODB-SERVER-IPADDRESS
```

# 3.e Show databases, Use database, Show Collections, Get the items in collection
```
show dbs
```
```
